...
def send_code_email(request):
    ...
    try:
        ...
        return Response({'message': f'發送失敗: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def register(request):
    ...
    VerificationCode.objects.filter(created_at__lt=valid_time).delete()  # 刪除過期驗證碼
...
def login(request):
    ...
    if user is not None:
        ...
        return Response({'message': '登入成功', 'username': user.username, 'token': token.key}, status=status.HTTP_200_OK)
    else:
        return Response({'message': '帳號或密碼錯誤'}, status=status.HTTP_401_UNAUTHORIZED)
